...
class SmartMCPServer:
    ...
    async def initialize(self) -> FastMCP:
        ...
        # Option 1: Lazy load in background (fast startup: ~10s)
        lazy_load = os.getenv('LAZY_LOAD_AI_SELECTORS', 'false').lower() == 'true'
        if lazy_load:
            print("🧠 AI selectors will initialize in background (LAZY_LOAD_AI_SELECTORS=true)...")
            # Start background task - doesn't block startup
            asyncio.create_task(self._initialize_ai_selectors_background())
        else:
            # Option 2: Synchronous load (default, slower startup: ~70s)
            await self._initialize_ai_selectors()

        print("✅ Smart MCP Server initialized successfully")
        return self.mcp

    async def _initialize_ai_selectors_background(self):
        """Initialize AI selectors in background without blocking startup"""
        try:
            # Wait for server to fully start
            await asyncio.sleep(5)
            logger.info("🧠 Starting background AI selector initialization...")
            print("🧠 Starting background AI selector initialization...")
            await self._initialize_ai_selectors()
            logger.info("✅ AI selectors ready (background initialization complete)")
            print("✅ AI selectors ready (background initialization complete)")
        except Exception as e:
            error_msg = f"❌ Background AI selector initialization failed: {e}"
    ...
    async def _initialize_core_services(self):
        """Initialize security, monitoring, database, and hardware SDK"""
        ...
        await self._initialize_hardware_sdk()
        
        print("✅ Core services ready")
    
    async def _initialize_hardware_sdk(self):
        """Initialize Hardware SDK components"""
        try:
            print("🔌 Initializing Hardware SDK...")
            
            # Import hardware SDK components
            from hardware_sdk.core.device_manager import get_device_manager
            
            # Initialize device manager
            device_manager = get_device_manager()
            await device_manager.initialize()
            
            print("  ✅ Hardware device manager initialized")
            print("  ✅ Hardware device discovery started")
            
        except ImportError as e:
            print(f"  ⚠️ Hardware SDK not available: {e}")
        except Exception as e:
            print(f"  ❌ Hardware SDK initialization failed: {e}")
            # Don't fail the entire server if hardware SDK fails
            logger.error(f"Hardware SDK initialization error: {e}")
...
def add_endpoints(app: Starlette):
    ...
    routes = [
        ...
        Route("/discover", discover_endpoint, methods=["POST", "OPTIONS"]),
        Route("/stats", stats_endpoint, methods=["GET", "OPTIONS"]),
        Route("/capabilities", capabilities_endpoint, methods=["GET", "OPTIONS"]),
        Route("/search", search_endpoint, methods=["POST", "OPTIONS"]),
        Route("/security/levels", security_levels_endpoint, methods=["GET", "OPTIONS"]),
        Route("/security/search", security_search_endpoint, methods=["POST", "OPTIONS"]),
...
async def run_server(port: int = 8081):
    ...
    logger.info("🚀 Starting Smart MCP Server with centralized logging")
    ...
    if os.getenv("CONSUL_ENABLED", "true").lower() == "true":
        try:
            ...
            logger.warning(f"⚠️ Consul registration failed: {e}, continuing without service discovery")
    ...
    # Configure auth services (use already imported get_settings)
    settings = get_settings()
    auth_config = {
    ...
    # Legacy auth middleware (kept for backward compatibility if needed)
