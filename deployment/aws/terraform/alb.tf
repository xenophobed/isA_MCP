...
resource "aws_lb_listener_rule" "user_service" {
  ...
  condition {
    path_pattern {
      values = ["/api/users/*", "/api/auth/*", "/api/billing/*"]
    }
  }
}
...
resource "aws_lb_listener_rule" "event_service" {
  ...
  condition {
    path_pattern {
      values = ["/api/events/*", "/api/analytics/*"]
    }
  }
}
...
output "target_group_arns" {
  value = {
    mcp_server    = aws_lb_target_group.mcp_server.arn
    user_service  = aws_lb_target_group.user_service.arn
    event_service = aws_lb_target_group.event_service.arn
  }
}