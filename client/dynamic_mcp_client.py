...
class DynamicMCPAgent:
    ...
    async def call_model(self, state: AgentState) -> AgentState:
        ...
        # Use a meta-prompt to let the LLM decide which prompt to use
...
Instructions:
1. First, determine if you need a specific prompt template by calling get_prompt_[name] with appropriate arguments
2. If you need additional context, access resources using get_resource_[name] 
3. If you need clarification from the user, use ask_human
4. If a tool requires authorization, use request_authorization first
5. Then use the appropriate MCP tools to fulfill the user's request

Be dynamic and intelligent in your approach - use the available MCP capabilities as needed."""
