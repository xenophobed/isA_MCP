...
from app.services.ai.mem.memory.memory_searcher import GraphSearcher as searcher
from app.services.ai.mem.graphs.tools import (
    ...
    async def _link_memory_ids(
        ...
        try:
            for memory_id in memory_ids:
                ...
                await self.graph.query(
                    memory_query,
                    params={"memory_id": memory_id, "user_id": filters["user_id"]}
                )
                
                # Link source and target to Memory
                link_query = """
                MATCH (n {name: $name, user_id: $user_id}), (m:Memory {id: $memory_id, user_id: $user_id})
