    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if resource_type:
            ...
            if not access_result.get("has_access"):
                logger.warning(f"❌ Access denied for {user_context.user_id} to {resource_type.value}/{resource_name}")
                return JSONResponse(
                    {
                        "error": "Access denied",
            ...
            logger.debug(f"✅ Access granted for {user_context.user_id} to {resource_type.value}/{resource_name}")
