...
class AuthorizationServiceClient:
    ...
    async def check_access(
        ...
        try:
            ...
            return {"has_access": False, "reason": str(e), "error": str(e)}

    async def check_bulk_access(
        self, user_id: str, resources: List[Dict[str, str]]
    ) -> Dict[str, Any]:
        """
        批量检查资源访问权限
        ...
        try:
            payload = {"user_id": user_id, "resources": resources}
    ...
    async def get_user_permissions(
        ...
        try:
            ...
            async with self.session.get(
                f"{self.base_url}/api/v1/auth/user-permissions", params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    # 转换权限格式
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    return {"success": False, "error": data.get("error", "Grant failed")}
    ...
    async def revoke_permission(
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    return {"success": True, "message": data.get("message", "Permission revoked")}
                else:
                    return {"success": False, "error": data.get("error", "Revoke failed")}
