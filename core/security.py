...
class AuthorizationManager:
    ...
    def create_request(
        ...
        request_id = hashlib.md5(
            f"{tool_name}:{json.dumps(arguments, sort_keys=True)}:{user_id}:{time.time()}".encode()
        ).hexdigest()
    ...
    def is_pre_approved(self, tool_name: str, arguments: Dict) -> bool:
        ...
        # First check cache
        cache_key = (
            f"{tool_name}:{hashlib.md5(json.dumps(arguments, sort_keys=True).encode()).hexdigest()}"
        )
        ...
        for request in self.pending_requests.values():
            if (
                ...
                and request.status == AuthorizationResult.APPROVED
...
class SecurityManager:
    ...
    def require_authorization(self, security_level: SecurityLevel = SecurityLevel.MEDIUM):
        ...
        def decorator(func):
            ...
            func._requires_authorization = security_level in [
                SecurityLevel.HIGH,
            ...
            if hasattr(wrapper, "__annotations__"):
                ...
                logger.debug(
                    f"Cleaned Context from security wrapper annotations for '{func.__name__}'"
                )
    ...
    def security_check(self, func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                # Rate limiting
                if self.monitoring_manager and not self.monitoring_manager.check_rate_limit(
                    tool_name, user_id, self.policy
                ):
                    if self.monitoring_manager:
                        self.monitoring_manager.log_request(
                            tool_name, user_id, False, 0, SecurityLevel.HIGH
                        )
                ...
                for pattern in self.policy.forbidden_patterns:
                    if re.search(pattern, content_to_check, re.IGNORECASE):
                        if self.monitoring_manager:
                            ...
                            self.monitoring_manager.log_request(
                                tool_name, user_id, False, 0, SecurityLevel.CRITICAL
                            )
                        raise SecurityViolationError(
                            f"Security violation: forbidden pattern detected"
                        )
                ...
                if self.monitoring_manager:
                    self.monitoring_manager.log_request(
                        tool_name, user_id, True, execution_time, security_level
                    )
                ...
                if self.monitoring_manager:
                    self.monitoring_manager.log_request(
                        tool_name, user_id, False, execution_time, security_level
                    )
                raise
