...
class MinIOClient:
    ...
    def _load_config(self) -> Dict[str, Any]:
        """Load MinIO configuration from environment variables with Consul discovery"""
        # Import consul_discovery for dynamic service discovery
        try:
            from core.consul_discovery import discover_service
            # Try to discover MinIO service from Consul, fallback to env var if discovery fails
            minio_endpoint = discover_service('minio', os.getenv('MINIO_ENDPOINT'))
            # Remove http:// prefix if present (MinIO client needs host:port format)
            ...
            logger.warning(f"Consul discovery failed for MinIO, using env var: {e}")
