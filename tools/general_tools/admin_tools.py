...
def register_admin_tools(mcp):
    ...
    async def approve_authorization(request_id: str, approved_by: str = "admin") -> str:
        ...
        logger.info(
            f"Authorization request {request_id} {'approved' if success else 'failed'} by {approved_by}"
        )
        return json.dumps(result)
    ...
    async def get_audit_log(limit: int = 50, user_id: str = "admin") -> str:
        ...
        recent_logs = (
            monitor_manager.request_history[-limit:]
            if limit > 0
            else monitor_manager.request_history
        )
