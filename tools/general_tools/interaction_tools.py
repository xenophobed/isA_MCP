...
def register_client_interaction_tools(mcp):
    ...
    async def request_authorization(
        tool_name: str, reason: str, user_id: str = "default", tool_args: dict | None = None
    ) -> str:
    ...
    async def check_security_status(include_metrics: bool = False, user_id: str = "default") -> str:
        ...
        try:
            ...
            pending_requests = len(
                [
                    req
                    for req in auth_manager.pending_requests.values()
    ...
    async def format_response(
        content: str, format_type: str = "structured", user_id: str = "default"
    ) -> str:
        ...
        try:
            if format_type == "json":
                ...
                if content.startswith("{") or content.startswith("["):
                    ...
                    formatted_content = json.dumps(
                        {"content": content, "formatted_at": datetime.now().isoformat()}
                    )
...
def register_interaction_tools(mcp):
    """Register interaction tools - alias for register_client_interaction_tools"""
    return register_client_interaction_tools(mcp)
