...
class TestEmbeddingGeneratorReal:
    ...
    async def test_compute_similarity(self, embedding_generator):
        ...
        try:
            ...
            ), f"Expected number, got {type(similarity_score)}"
            assert (
                0.0 <= similarity_score <= 1.0
            ), f"Similarity should be 0-1, got {similarity_score}"
    ...
    async def test_find_most_similar(self, embedding_generator):
        ...
        try:
            ...
            assert all(
                isinstance(score, (int, float)) for _, score in result
    ...
    async def test_chunk_text(self, embedding_generator):
        ...
        try:
            ...
            result = await embedding_generator.chunk_text(
                long_text.strip(), chunk_size=100, overlap=20
            )
...
class TestConvenienceFunctionsReal:
    ...
    async def test_chunk_convenience_function(self):
        ...
        try:
            result = await chunk(
                "This is a long document that needs chunking for better processing.", chunk_size=50
            )
