...
class TestTextExtractor:
    ...
    async def test_extract_entities(self):
        ...
        result = await self.extractor.extract_entities(text=data, confidence_threshold=0.6)
    ...
    async def test_classify_text(self):
        ...
        result = await self.extractor.classify_text(
            text=data, categories=categories, multi_label=False
        )

        success = (
            result["success"]
            and "data" in result
            and "primary_category" in result["data"]
            and result["confidence"] > 0.5
        )
    ...
    async def test_extract_key_information(self):
        ...
        result = await self.extractor.extract_key_information(text=data, schema=schema)

        success = (
            result["success"]
            and "data" in result
            and result["confidence"] > 0.4
            and result["completeness"] > 0.3
        )
    ...
    async def test_summarize_text(self):
        ...
        success = (
            result["success"]
            and "data" in result
            and "summary" in result["data"]
            and len(result["data"]["summary"]) > 50
            and result["confidence"] > 0.5
        )
    ...
    async def test_analyze_sentiment(self):
        ...
        data = self.get_test_data()["customer_review"]
        result = await self.extractor.analyze_sentiment(text=data, granularity="overall")

        success = (
            result["success"]
            and "data" in result
            and "overall_sentiment" in result["data"]
            and result["confidence"] > 0.5
        )
    ...
    async def test_multi_label_classification(self):
        ...
        result = await self.extractor.classify_text(
            text=data, categories=categories, multi_label=True
        )

        success = (
            result["success"]
            and "data" in result
            and "classification" in result["data"]
            and result["confidence"] > 0.5
        )
    ...
    async def test_convenience_functions(self):
        ...
        success = result["success"] and "data" in result and result["confidence"] > 0.3
    ...
    async def run_all_tests(self):
        ...
        for test_result in self.test_results:
            ...
            print(
                f"{status} {test_result['test']:<25} | Confidence: {test_result['confidence']:.3f} | Cost: ${cost:.6f}"
            )
