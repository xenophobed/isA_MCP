...
Create the summary according to the specified style and length."""
    ...
    async def extract_key_points(
        self, text: str, max_points: int = 10, temperature: float = 0.1, max_tokens: int = 800
    ) -> Dict[str, Any]:
        """
...
{text_content}"""
            ...
            if billing_info:
                logger.info(
                    f"💰 Key points extraction cost: ${billing_info.get('cost_usd', 0.0):.6f}"
                )
...
Return your analysis in a clear, organized format."""
    ...
    def _calculate_summary_quality(
        self, summary: str, original: str, style: SummaryStyle, length: SummaryLength
    ) -> float:
