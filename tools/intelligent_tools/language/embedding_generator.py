...
async def hybrid_search_local(
    ...
    **kwargs
    ...
    try:
        ...
        from tools.services.intelligence_service.vector_db.base_vector_db import VectorSearchConfig, SearchMode, RankingMethod
        ...
        logger.info(f"Hybrid search returned {len(results)} results using {search_result.get('method')}")
        return results
    ...
    **kwargs
    ...
    try:
        ...
        if candidates:
            ...
            logger.debug(f"Enhanced search using ISA with candidates: {len(enhanced_results)} results")
        ...
        if use_local_db:
            ...
            if local_results:
                logger.debug(f"Enhanced search fallback to local semantic: {len(local_results)} results")
                return local_results
...
async def mmr_rerank(
    ...
    **kwargs
    ...
    try:
        ...
        logger.info(f"MMR reranking: {len(documents)} → {len(reranked_docs)} results, λ={lambda_param}")
        ...
        if method == "mmr":
            ...
            for result in isa_results:
                ...
                if doc_text in combined_scores:
                    combined_scores[doc_text]['isa_score'] = result.get('relevance_score', result.get('score', 0.0))
                else:
                    combined_scores[doc_text] = {
                        'mmr_score': 0.0,
    ...
    **kwargs
    ...
    try:
        ...
        return {
            ...
            'average_time_per_file': total_processing_time / len(successful_files) if successful_files else 0,
            'parallel_efficiency': (total_processing_time / overall_time) if overall_time > 0 else 0,
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.error("No PDF library available. Install: pip install PyPDF2 or pdfplumber or PyMuPDF")
                    return ""
...
async def _extract_text_file_streaming(file_path: str, **kwargs) -> str:
    ...
    try:
        ...
        file_size = os.path.getsize(file_path)
        if file_size > max_size:
            logger.warning(f"File {file_path} too large ({file_size / 1024 / 1024:.1f}MB), truncating")
...
async def process_mixed_files(
    ...
    **kwargs
    ...
    return await process_large_files_parallel(
        ...
        user_id=user_id,
        file_types=None,  # Allow all types
        **kwargs
    )