...
from tools.intelligent_tools.vision.image_analyzer import (
    ImageAnalyzer,
    ImageAnalysisResult,
    analyze,
)
...
class TestImageAnalyzer:
    ...
    async def test_analyze_with_bytes_input(self):
        ...
        try:
            result = await self.analyzer.analyze(
                image=image_bytes, prompt="What color is this image?", provider="openai"
            )
    ...
    async def test_analyze_different_prompts(self):
        ...
        for i, prompt in enumerate(test_prompts):
            try:
                ...
                result = await self.analyzer.analyze(
                    image=image_path, prompt=prompt, provider="openai"
                )
    ...
    async def test_analyze_invalid_image_input(self):
        ...
        try:
            result = await self.analyzer.analyze(image=12345, prompt="Test prompt")  # Invalid type
    ...
    async def test_convenience_function(self):
        ...
        try:
            result = await analyze(image=image_path, prompt="What do you see?", provider="openai")
