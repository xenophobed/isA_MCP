...
from tools.services.web_services.strategies.detection.enhanced_vision_analyzer import (
...
class EnhancedVisionTester:
    ...
    async def test_reddit_dynamic_search(self):
        ...
        try:
            ...
            search_elements = await self.enhanced_analyzer.adaptive_search_detection(
                page, deep_analysis
            )
    ...
    async def _execute_dynamic_search(
        self, page, search_elements: dict, page_analysis: dict
    ) -> bool:
        """根据页面分析结果执行动态搜索"""
        try:
            ...
            # 获取搜索策略
            search_trigger = page_analysis.get("search_analysis", {}).get(
                "search_trigger", "unknown"
            )
            ...
            if search_trigger == "enter_key":
                ...
                # 查找搜索按钮
                search_button = search_elements.get("submit") or search_elements.get(
                    "search_button"
                )
                if search_button:
