...
class TriggerCondition(BaseModel):
    ...
    event_type: Optional[str] = Field(None, description="Event type to monitor (e.g., price_change, news_published)")
    event_source: Optional[str] = Field(None, description="Event source (frontend, backend, system, iot_device)")
    event_category: Optional[str] = Field(None, description="Event category (user_action, system, alert)")
    threshold_type: Optional[str] = Field(None, description="Threshold type: percentage or absolute")
    ...
    product: Optional[str] = Field(None, description="Product/asset to monitor (e.g., Bitcoin, AAPL stock)")
    keywords: Optional[List[str]] = Field(None, description="Keywords to monitor in events")
    schedule: Optional[Dict[str, Any]] = Field(None, description="Schedule configuration for time-based triggers")


class ActionConfig(BaseModel):
    """Action configuration when trigger fires"""
    action: str = Field(..., description="Action to perform: generate_summary, price_alert, event_notification, custom")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Action-specific parameters")
    notification_channels: Optional[List[str]] = Field(None, description="Notification channels: email, sms, push")
...
class EventTools(BaseTool):
    ...
    async def register_price_alert_impl(
        self,
        product: Annotated[str, Field(description="Product name to monitor (e.g., Bitcoin, AAPL, Tesla)")],
        threshold_value: Annotated[float, Field(description="Price change threshold (e.g., 50 for 50%)", gt=0)],
        threshold_type: Annotated[str, Field(description="Threshold type: 'percentage' or 'absolute'")] = "percentage",
        direction: Annotated[str, Field(description="Direction: 'up', 'down', or 'any'")] = "any",
        notification_channels: Annotated[Optional[List[str]], Field(description="Channels: email, sms, push")] = None,
        user_id: str = "default",
        ...
        """
        await self.log_info(ctx, f"Registering price alert: {product} {threshold_type} {direction} {threshold_value}")
        await self.report_progress(ctx, 1, 4, "Validating inputs")

        # Validate inputs
        if threshold_type not in ["percentage", "absolute"]:
            return self.create_response(
                "error", "register_price_alert",
    ...
    async def register_scheduled_task_impl(
        self,
        task_name: Annotated[str, Field(description="Task name (e.g., 'Daily News Summary', 'Weekly Report')")],
        task_type: Annotated[str, Field(description="Task type: daily_news, daily_weather, custom")] = "custom",
        schedule_type: Annotated[str, Field(description="Schedule type: daily, weekly, monthly, cron")] = "daily",
        schedule_time: Annotated[str, Field(description="Time in HH:MM format (e.g., '09:00')")] = "09:00",
        schedule_day: Annotated[Optional[int], Field(description="Day for weekly (1-7) or monthly (1-31)")] = None,
        action_parameters: Annotated[Optional[Dict[str, Any]], Field(description="Action-specific parameters")] = None,
        user_id: str = "default",
        ctx: Optional[Context] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        await self.log_info(ctx, f"Registering scheduled task: {task_name} ({schedule_type} at {schedule_time})")
        await self.report_progress(ctx, 1, 4, "Validating schedule")

        # Validate schedule type
        if schedule_type not in ["daily", "weekly", "monthly", "cron"]:
            return self.create_response(
                "error", "register_scheduled_task",
        ...
        # Build action config
        action_config = {
            "action": "generate_summary" if "summary" in task_name.lower() or "news" in task_type.lower() else "custom",
    ...
    async def register_event_trigger_impl(
        self,
        trigger_name: Annotated[str, Field(description="Trigger name (e.g., 'Security Alert', 'Failed Login Monitor')")],
        event_type: Annotated[str, Field(description="Event type to monitor")],
        event_source: Annotated[Optional[str], Field(description="Event source: frontend, backend, system")] = None,
        event_category: Annotated[Optional[str], Field(description="Event category")] = None,
        keywords: Annotated[Optional[List[str]], Field(description="Keywords to match in event data")] = None,
        action_type: Annotated[str, Field(description="Action type: event_notification, custom")] = "event_notification",
        user_id: str = "default",
        ctx: Optional[Context] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self.create_response(
                    "success", "register_event_trigger",
                    {
                        "trigger_id": trigger_id,
    ...
    async def query_events_impl(
        ...
        try:
            ...
            response = await self.http_client.post(
                f"{self.event_service_url}/api/events/query",
                json=params
            )
