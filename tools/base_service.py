...
class BaseService:
    ...
    async def call_isa_with_billing(
        ...
        try:
            ...
            if billing_info:
                ...
                logger.info(
                    f"💰 {self.service_name} - {operation_name or task}: ${billing_info.get('cost_usd', 0.0):.6f}"
                )
    ...
    def get_billing_summary(self) -> Dict[str, Any]:
        ...
        if not self.billing_info:
            return {"total_cost_usd": 0.0, "operation_count": 0, "operations": []}
    ...
    def create_batch_response(
        self, operation: str, results: List[Any], errors: List[str] = None
    ) -> str:
        """创建批量操作响应"""
