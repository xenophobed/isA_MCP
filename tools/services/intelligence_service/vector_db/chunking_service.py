...
class ChunkConfig:
    ...
    metadata_fields: List[str] = field(
        default_factory=lambda: ["position", "source", "chunk_id", "parent_id"]
    )
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        while start < text_length:
            ...
            if len(chunk_text.strip()) > 0:
                chunks.append(self._create_chunk(chunk_text, position, start, end, metadata))
                position += 1
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        for sentence in sentences:
            ...
            if current_length + sentence_length > self.config.chunk_size and current_chunk:
                ...
                chunks.append(
                    self._create_chunk(chunk_text, position, start_char, char_position, metadata)
                )
                position += 1
...
class RecursiveChunker(BaseChunker):
    ...
    async def _recursive_split(
        self, text: str, separators: List[str], metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
                ...
                for i, part in enumerate(parts):
                    ...
                    if len(part) > self.config.chunk_size:
                        sub_chunks = await self._recursive_split(
                            part, separators[separators.index(separator) + 1 :], metadata
                        )
    ...
    def _merge_small_chunks(self, chunks: List[Chunk]) -> List[Chunk]:
        ...
        for next_chunk in chunks[1:]:
            ...
            if (
                combined_length <= self.config.chunk_size
...
class MarkdownChunker(BaseChunker):
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        for section in sections:
            ...
            if len(section["content"]) <= self.config.chunk_size:
                ...
                chunk_metadata = metadata or {}
                chunk_metadata.update(
                    {"section_level": section["level"], "section_title": section["title"]}
                )
    ...
    def _split_by_headers(self, text: str) -> List[Dict[str, Any]]:
        ...
        sections = []
        current_section = {"level": 0, "title": "Introduction", "content": "", "start": 0, "end": 0}
    ...
    async def _split_large_section(
        self, section: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
        ...
        for para in paragraphs:
            ...
            if current_length + para_length > self.config.chunk_size and current_chunk:
                ...
                chunks.append(
                    self._create_chunk(current_chunk, 0, 0, len(current_chunk), chunk_metadata)
                )
...
class CodeChunker(BaseChunker):
    ...
    def __init__(self, config: ChunkConfig):
        ...
        self.function_patterns = {
            ...
            "java": re.compile(
                r"^(public|private|protected)?\s*(static\s+)?[\w<>]+\s+\w+\([^)]*\)", re.MULTILINE
            ),
        }
        self.class_patterns = {
    ...
    async def _chunk_by_structure(
        self, text: str, language: str, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
        ...
        if language in self.class_patterns:
            for match in self.class_patterns[language].finditer(text):
                structures.append(
                    {"type": "class", "start": match.start(), "match": match.group(0)}
                )
        ...
        if language in self.function_patterns:
            for match in self.function_patterns[language].finditer(text):
                structures.append(
                    {"type": "function", "start": match.start(), "match": match.group(0)}
                )
        ...
        for i, struct in enumerate(structures):
            ...
            if len(content) > self.config.chunk_size:
                ...
                chunk_metadata = metadata or {}
                chunk_metadata.update({"code_type": struct["type"], "signature": struct["match"]})

                chunks.append(
                    self._create_chunk(content, position, struct["start"], end, chunk_metadata)
                )
                position += 1
        ...
        if last_end < len(text):
            ...
            if remainder.strip():
                ...
                chunks.append(
                    self._create_chunk(remainder, position, last_end, len(text), chunk_metadata)
                )
    ...
    async def _chunk_by_lines(
        ...
        for line in lines:
            ...
            if current_size + line_size > self.config.chunk_size and current_chunk:
                ...
                chunk_text = "\n".join(current_chunk)
                chunks.append(
                    self._create_chunk(chunk_text, position, chunk_start, char_position, metadata)
                )
                position += 1
    ...
    async def _split_large_code_block(
        self, content: str, code_type: str, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        try:
            embeddings = await embedder.embed_batch(
                sentence_texts, model=self.config.embedding_model
            )
        except Exception as e:
            logger.warning(
                f"Failed to generate embeddings: {e}. Falling back to sentence chunking."
            )
            return sentence_chunks

        # Group sentences by semantic similarity
        grouped_chunks = await self._group_by_similarity(sentence_chunks, embeddings, metadata)
    ...
    async def _group_by_similarity(
        ...
        for i in range(1, len(sentences)):
            ...
            should_add = (
                ...
                and current_size + len(sentence.text) <= self.config.chunk_size
            )
            ...
            if should_add:
                ...
                chunk_metadata = metadata or {}
                chunk_metadata.update(
                    {"sentence_count": len(current_group), "avg_similarity": similarity}
                )

                chunks.append(
                    self._create_chunk(chunk_text, position, start_char, end_char, chunk_metadata)
                )
                position += 1
...
class TokenChunker(BaseChunker):
    ...
    def _init_tokenizer(self):
        ...
        try:
            ...
            logger.info(
                "tiktoken not available - using character-based approximation for token chunking"
            )
            self.tokenizer = None
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        while start_idx < total_tokens:
            ...
            chunks.append(self._create_chunk(chunk_text, position, start_char, end_char, metadata))
            position += 1
    ...
    async def _approximate_token_chunking(
        ...
        temp_config = ChunkConfig(
            strategy=ChunkingStrategy.FIXED_SIZE, chunk_size=char_limit, chunk_overlap=char_overlap
        )
...
class HierarchicalChunker(BaseChunker):
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        root_chunk = self._create_chunk(
            ...
            0,
            0,
            min(len(text), self.config.max_chunk_size),
            metadata,
        )
...
class ChunkingService:
    ...
    async def chunk_text(
        ...
        **kwargs,
        ...
        logger.info(
            f"Chunked text using {strategy.value}: " f"{len(text)} chars -> {len(chunks)} chunks"
        )
    ...
    async def chunk_document(
        ...
        **kwargs,
        ...
        return await self.chunk_text(text=text, strategy=strategy, metadata=metadata, **kwargs)
        ...
        async def chunk_with_semaphore(text: str, index: int) -> List[Chunk]:
            async with semaphore:
                ...
                return await self.chunk_text(
                    text=text, strategy=strategy, metadata=metadata, **kwargs
                )

        tasks = [chunk_with_semaphore(text, i) for i, text in enumerate(texts)]
...
async def chunk(
    ...
    **kwargs,
    ...
    chunks = await chunking_service.chunk_text(
        text=text, strategy=strategy, chunk_size=chunk_size, chunk_overlap=chunk_overlap, **kwargs
    )
...
async def smart_chunk(text: str, **kwargs) -> List[Dict[str, Any]]:
    ...
    chunks = await chunking_service.chunk_text(text=text, strategy=strategy, **kwargs)
