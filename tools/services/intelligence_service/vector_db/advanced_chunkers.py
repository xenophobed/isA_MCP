    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        for para in paragraphs:
            ...
            if current_chunk_text and (
                len(current_chunk_text) + para_length > self.config.chunk_size
                or (
                    len(current_chunk_text) > self.config.min_chunk_size
                    and self._is_topic_boundary(current_chunk_text, para)
                )
            ):
                # Save current chunk
                chunks.append(
        ...
        if current_chunk_text:
            chunks.append(
                self._create_chunk(
                    current_chunk_text, position, current_chunk_start, char_position, metadata
                )
            )
...
class TopicChunker(BaseChunker):
        ...
        if self.config.use_embeddings:
            try:
                ...
                embeddings = await embedder.embed_batch(
                    sentences, model=self.config.embedding_model
                )
    ...
    def _detect_topic_boundaries(
        self, sentences: List[str], embeddings: Optional[List[List[float]]] = None
    ) -> List[int]:
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        while start < text_length:
            ...
            if chunk_text:
                ...
                chunks.append(self._create_chunk(chunk_text, position, start, end, chunk_metadata))
                position += 1
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        for table_start, table_end, table_text in tables:
            ...
            if len(table_text) <= self.config.chunk_size:
                ...
                chunks.append(
                    self._create_chunk(table_text, position, table_start, table_end, table_metadata)
                )
                position += 1
            else:
...
class ConversationChunker(BaseChunker):
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        for turn in turns:
            ...
            if current_size + turn_size > self.config.chunk_size and current_chunk_turns:
                ...
                chunk_text = "\n".join(
                    [f"{t['speaker']}: {t['message']}" for t in current_chunk_turns]
                )
                ...
                chunks.append(
                    self._create_chunk(chunk_text, position, 0, len(chunk_text), conv_metadata)
                )
                position += 1
        ...
        if current_chunk_turns:
            chunk_text = "\n".join([f"{t['speaker']}: {t['message']}" for t in current_chunk_turns])
    ...
    def _extract_turns(self, text: str) -> List[Dict[str, Any]]:
        ...
        # Add final turn
        if current_speaker and current_message:
            turns.append({"speaker": current_speaker, "message": " ".join(current_message).strip()})
...
class JSONChunker(BaseChunker):
    ...
    async def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> List[Chunk]:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                # Simple value
                return [self._create_chunk(json.dumps(data, indent=2), 0, 0, len(text), metadata)]
    ...
    async def _chunk_json_array(
        self, data: list, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
        """Chunk JSON array"""
        import json
        ...
        for item in data:
            ...
            if current_size + item_size > self.config.chunk_size and current_items:
                ...
                json_metadata.update({"json_type": "array", "item_count": len(chunk_data)})

                chunks.append(
                    self._create_chunk(chunk_json, position, 0, len(chunk_json), json_metadata)
                )
                position += 1
        ...
        if current_items:
            ...
            json_metadata = metadata or {}
            json_metadata.update({"json_type": "array", "item_count": len(current_items)})
    ...
    async def _chunk_json_object(
        self, data: dict, metadata: Optional[Dict[str, Any]] = None
    ) -> List[Chunk]:
        """Chunk JSON object"""
        import json
