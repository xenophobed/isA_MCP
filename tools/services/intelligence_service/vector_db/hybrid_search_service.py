        ...
        try:
            if self.vector_db:
                success = await self.vector_db.store_vector(
                    id=id, text=text, embedding=embedding, user_id=user_id, metadata=metadata
                )

                return {"success": success, "method": "local_db", "id": id, "user_id": user_id}
            else:
                return {
                    "success": False,
        ...
        try:
            ...
            if self.vector_db and search_config.search_mode == SearchMode.HYBRID:
                try:
                    ...
                    if local_results:
                        self.logger.debug(
                            f"Local hybrid search returned {len(local_results)} results"
                        )
                        return {
                            "success": True,
            ...
            # Local vector search
            if self.vector_db and search_config.search_mode in [
                SearchMode.SEMANTIC,
                SearchMode.HYBRID,
            ]:
    ...
    async def _local_semantic_search(
        ...
        try:
            ...
            results = await self.vector_db.search_vectors(
                query_embedding=query_embedding, user_id=user_id, config=config
            )

            return {"method": "local_semantic", "results": results, "success": True}
    ...
    async def _local_text_search(
        ...
        try:
            ...
            results = await self.vector_db.search_text(
                query_text=query_text, user_id=user_id, config=config
            )

            return {"method": "local_text", "results": results, "success": True}
        ...
        try:
            ...
            if self.vector_db:
                ...
                all_vectors = await self.vector_db.list_vectors(
                    user_id=user_id, limit=1000  # Reasonable limit for ISA processing
                )
                ...
                if all_vectors:
                    ...
                    isa_results = await embedding_generator.find_most_similar(
                        query_text=query_text, candidate_texts=candidate_texts, top_k=config.top_k
                    )
                    ...
                    for text, similarity_score in isa_results:
                        ...
                        matching_vector = next((v for v in all_vectors if v.text == text), None)
            ...
            return {"method": "isa_fallback", "results": [], "error": str(e)}
