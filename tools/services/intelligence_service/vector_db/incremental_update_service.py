...
class IncrementalUpdateService:
    ...
    async def record_change(
        ...
        try:
            ...
            if (
                self.policy.enable_auto_reindex and 
                self.change_count >= self.policy.reindex_threshold_changes
            ):
    ...
    async def process_pending_changes(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            should_process = (
                force or
                len(self.pending_changes) >= self.policy.batch_size or
                time_since_last >= self.policy.batch_interval_minutes
            )
            ...
            self.logger.info(f"Processed {total_processed} changes in {len(grouped_changes)} groups")
