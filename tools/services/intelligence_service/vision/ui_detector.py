    ...
    async def _match_requirements_with_elements(
        ...
        try:
            ...
            for req in requirements:
                requirements_text += f"NEED: {req.get('element_name')} - {req.get('element_purpose')}\\n"
                ...
                for req in requirements:
                    ...
                    if element_name in matches:
                        ...
                        if 0 <= elem_idx < len(ui_elements):
                            ...
                            action = 'click'
                            if 'type' in interaction_type.lower() or 'input' in element_name.lower():
                                action = 'type'
                            elif 'click' in interaction_type.lower() or 'button' in element_name.lower():
                                action = 'click'
                            ...
                            logger.info(f"✅ Matched {element_name}: Element {elem_idx} at ({center[0]}, {center[1]})")
...
async def detect_ui_with_coordinates(
    ...
    return await ui_detector.detect_ui_with_coordinates(screenshot, requirements, url, force_refresh)
