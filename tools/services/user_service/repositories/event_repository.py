...
class EventRepository(BaseRepository):
    ...
    def __init__(self):
        super().__init__("events")  # 传递表名给BaseRepository
    ...
    async def append_event(self, event_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            event_record = {
                'event_id': str(event_id),  # Convert UUID to string
            ...
            result = self.table.insert(event_record).execute()
    ...
    async def get_event_stream(self, stream_id: str, from_version: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            query = self.table.select('*').eq('task_id', stream_id)
    ...
    async def get_event_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_events = self.table.select('event_type, processed').execute()
    ...
    # 实现BaseRepository抽象方法
    async def create(self, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """创建新事件 - 实现BaseRepository抽象方法"""
        event_id = await self.append_event(data)
        if event_id:
            return await self.get_by_id(event_id)
        return None
    
    async def get_by_id(self, event_id: Any) -> Optional[Dict[str, Any]]:
        """根据ID获取事件 - 实现BaseRepository抽象方法"""
        try:
            result = self.table\
                .select('*')\
            ...
            self.logger.error(f"Failed to get event by ID {event_id}: {e}")
            return None
    
    async def update(self, event_id: Any, data: Dict[str, Any]) -> bool:
        """更新事件 - 实现BaseRepository抽象方法"""
        try:
            # Add timestamp
    ...
    async def delete(self, event_id: Any) -> bool:
        """删除事件 - 实现BaseRepository抽象方法"""
        try:
            result = self.table\
                .delete()\
                .eq('event_id', str(event_id))\
