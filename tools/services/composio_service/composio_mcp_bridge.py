...
class ComposioMCPBridge:
    ...
    async def register_composio_tools_with_mcp(self, mcp):
        ...
        try:
            ...
            logger.warning("Security manager not initialized, registering tools without security decorators")
        ...
        try:
            ...
            if result.get("success"):
                ...
                priority_apps = [
                    "gmail", "slack", "github", "notion", "googlecalendar",
                    "discord", "telegram", "asana", "trello", "dropbox",
                    "googledrive", "hubspot", "salesforce", "jira", "linear",
                    "mailchimp", "airtable", "zoom", "stripe", "shopify"
                ]
                ...
                for app in apps:
                    ...
                    if app_name in priority_apps:
                        app_tools_metadata = await self._register_app_as_tools(mcp, app, security_manager)
                        tools_metadata.update(app_tools_metadata)
                        registered_count += 1
                        logger.info(f"  ✅ Registered {app_name} ({registered_count}/{len(priority_apps)})")
                
                logger.info(f"  🎉 Successfully registered {registered_count} priority Composio apps!")
    ...
    async def _register_management_tools(self, mcp, security_manager):
        ...
        if security_manager:
            composio_connect_app = security_manager.require_authorization(SecurityLevel.LOW)(composio_connect_app)
        ...
        if security_manager:
            composio_list_user_connections = security_manager.require_authorization(SecurityLevel.LOW)(composio_list_user_connections)
        ...
        async def composio_list_available_apps() -> str:
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    return json.dumps({
                        "status": "error",
                        "message": result.get("error", "Failed to list apps")
                    }, indent=2)
        ...
        if security_manager:
            composio_list_available_apps = security_manager.require_authorization(SecurityLevel.LOW)(composio_list_available_apps)
    ...
    async def _register_app_as_tools(self, mcp, app_info: Dict, security_manager):
        ...
        tool2_meta = await self._create_dynamic_tool(
            ...
            app_name,
            "get_data", 
            f"Get data from {app_name.title()}",
            security_manager
        )
        if tool2_meta:
    ...
    async def _create_dynamic_tool(self, mcp, app_name: str, action: str, description: str, security_manager):
        ...
        async def dynamic_composio_tool(parameters: Dict[str, Any] = None, user_id: str = "default") -> str:
            """Dynamic Composio tool wrapper"""
            try:
                # Check if user has connected this app
                if user_id not in self.user_connections or app_name not in self.user_connections[user_id]:
        ...
        # Register with MCP
        if security_manager:
            security_level = SecurityLevel.MEDIUM if "send" in action or "create" in action else SecurityLevel.LOW
            dynamic_composio_tool = security_manager.require_authorization(security_level)(dynamic_composio_tool)
