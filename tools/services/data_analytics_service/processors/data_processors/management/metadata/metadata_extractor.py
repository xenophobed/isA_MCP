...
try:
    ...
    # Fallback - create minimal processor if needed
    class CSVProcessor:
...
class MetadataExtractor:
    ...
    def __init__(self):
        self.supported_sources = {
            ...
            'duckdb': self._extract_duckdb_metadata
        }
    ...
    def _extract_duckdb_metadata(self, db_path: str, **kwargs) -> Dict[str, Any]:
        """
        Extract metadata from DuckDB database
        ...
        try:
            ...
            metadata = {
                "file_info": {
                    ...
                    "file_path": db_path,
                    "file_type": "duckdb",
                    "file_size_mb": round(Path(db_path).stat().st_size / (1024 * 1024), 2) if Path(db_path).exists() else 0
                },
                "database_info": {
            ...
            for table_name in table_names:
                try:
                    ...
                    # Get table columns
                    table_columns = [col for col in columns_info if col[0] == table_name]
                    ...
                    for col_info in table_columns:
                        col_metadata = {
                            ...
                            "is_nullable": col_info[3] == 'YES',
                            "ordinal_position": col_info[4],
                            "table_name": table_name
                        }
                        
                        # Try to get sample values (limit to avoid performance issues)
                        try:
                            sample_query = f"SELECT DISTINCT {col_info[1]} FROM {table_name} LIMIT 5"
                            sample_results = conn.execute(sample_query).fetchall()
                            col_metadata["sample_values"] = [str(row[0]) for row in sample_results if row[0] is not None]
                        except Exception:
            ...
            metadata["summary"] = {
                ...
                "total_records": sum(table["record_count"] for table in metadata["tables"])
            }
            
            logger.info(f"Extracted metadata from DuckDB: {len(table_names)} tables, {len(metadata['columns'])} columns")
            return metadata
            
        except ImportError:
            return {
                "error": "DuckDB module not available. Please install duckdb: pip install duckdb",
                "file_path": db_path
            }
        except Exception as e:
            logger.error(f"Failed to extract DuckDB metadata: {e}")
            return {
                "error": f"Failed to extract DuckDB metadata: {str(e)}",
                "file_path": db_path
            }
    ...
    def _standardize_metadata(self, raw_metadata: Dict[str, Any], source_type: str) -> Dict[str, Any]:
        ...
        if source_type == "csv":
            ...
            standardized.update(self._standardize_duckdb_metadata(raw_metadata))
    ...
    def _standardize_duckdb_metadata(self, duckdb_metadata: Dict[str, Any]) -> Dict[str, Any]:
        """Standardize DuckDB metadata"""
        if "error" in duckdb_metadata:
            return {"error": duckdb_metadata["error"]}
        
        file_info = duckdb_metadata.get("file_info", {})
        tables = duckdb_metadata.get("tables", [])
        columns = duckdb_metadata.get("columns", [])
        summary = duckdb_metadata.get("summary", {})
        ...
        for table in tables:
            standardized_table = {
                ...
                "database_path": file_info.get("file_path")
            }
            standardized_tables.append(standardized_table)
        
        # Transform columns to standardized format
        standardized_columns = []
        for col in columns:
            standardized_col = {
        ...
        return {
            ...
            "columns": standardized_columns,
            "sample_data": []  # Could be populated if needed
        }
