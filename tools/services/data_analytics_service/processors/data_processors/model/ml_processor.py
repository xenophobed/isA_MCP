...
# Core ML libraries - LAZY LOADING TO PREVENT HANGING
def _check_sklearn():
...
def _check_prophet():
    """Lazy check for Prophet availability"""
    try:
        import prophet
        return True
    except ImportError:
...
# Other processors - LAZY LOADING TO AVOID CIRCULAR IMPORTS
def _lazy_import_processors():
    """Lazy import other processors to avoid circular dependencies"""
    try:
...
class MLProcessor:
    ...
    def __init__(self, csv_processor: Optional[Any] = None, file_path: Optional[str] = None):
        """
    ...
    def recommend_algorithms(self, target_column: str, problem_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            target_series = self.df[target_column].dropna() if target_column in self.df.columns else self.df.get('y', self.df.iloc[:, -1]).dropna()
            ...
            if problem_type == "time_series":
                ...
                if PROPHET_AVAILABLE:
                    basic_algorithms.append({
                        "name": "Prophet",
                        "library": "prophet",
                        "pros": ["Handles seasonality automatically", "Robust to missing data", "Intuitive hyperparameters"],
                        "cons": ["Requires ds and y column format", "May overfit with limited data"],
                        "best_for": "Time series forecasting with seasonality",
                        "priority": "high"
                    })
                
                # Also recommend traditional regression for comparison
                basic_algorithms.extend([
                    {
                        "name": "Linear Regression",
                        "sklearn_class": "LinearRegression",
                        "pros": ["Simple baseline", "Fast", "Interpretable"],
                        "cons": ["Assumes linear trend", "No seasonality handling"],
                        "best_for": "Simple trend forecasting",
                        "priority": "medium"
                    },
                    {
                        "name": "Random Forest Regressor",
                        "sklearn_class": "RandomForestRegressor",
                        "pros": ["Non-linear relationships", "Feature importance"],
                        "cons": ["No inherent time series structure"],
                        "best_for": "Feature-based time series prediction",
                        "priority": "low"
                    }
                ])
                
                recommendations["recommended_algorithms"] = basic_algorithms
                recommendations["time_series_note"] = "Prophet is specifically designed for time series and is recommended over general regression algorithms"
...
""",
    ...
    def train_model(self, target_column: str, algorithm: str, test_size: float = 0.2, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            problem_type = self._detect_problem_type(self.df[target_column] if target_column in self.df.columns else self.df.get('y', self.df.iloc[:, -1]))
            
            # Handle Prophet time series separately
            if algorithm.lower() == "prophet" and problem_type == "time_series":
                return self._train_prophet_model(algorithm, test_size, **kwargs)
    ...
    def _train_prophet_model(self, algorithm: str, test_size: float = 0.2, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'ds' not in self.df.columns or 'y' not in self.df.columns:
                return {"error": "Prophet requires 'ds' (date) and 'y' (target) columns"}
            
            # Prepare Prophet data
            prophet_df = self.df[['ds', 'y']].copy()
            ...
            metrics = self._calculate_time_series_metrics(test_actual, test_predictions)
            ...
            results = {
                ...
                "metrics": metrics,
                "prophet_components": {
                    "trend": forecast['trend'].iloc[-len(test_df):].tolist()[:5],  # Sample trend values
                    "has_yearly_seasonality": model.yearly_seasonality,
                    "has_weekly_seasonality": model.weekly_seasonality,
                    "has_daily_seasonality": model.daily_seasonality
                },
                "forecast_period": f"{test_df['ds'].min()} to {test_df['ds'].max()}",
    ...
    def _calculate_time_series_metrics(self, y_true, y_pred) -> Dict[str, float]:
        ...
        # Mean Absolute Scaled Error (MASE) - requires historical data for naive forecast
        # For now, we'll use a simplified version
        naive_forecast = np.roll(y_true, 1)[1:]  # Simple lag-1 forecast
        actual_subset = y_true[1:]
        if len(naive_forecast) > 0 and len(actual_subset) > 0:
            mae_naive = np.mean(np.abs(actual_subset - naive_forecast))
            if mae_naive != 0:
                mase = metrics["mean_absolute_error"] / mae_naive
                metrics["mean_absolute_scaled_error"] = float(mase)
        
        return metrics
    
    def _detect_problem_type(self, target_series: pd.Series) -> str:
        """Detect if problem is classification, regression, or time series"""
        # Check if data has Prophet format columns (ds, y)
        if hasattr(self, 'df') and 'ds' in self.df.columns and 'y' in self.df.columns:
            return "time_series"
    ...
    def _get_model(self, algorithm: str, **kwargs):
        ...
        # Add Prophet for time series if available
        if PROPHET_AVAILABLE:
            model_map.update({
                "prophet": Prophet(**kwargs)
            })
