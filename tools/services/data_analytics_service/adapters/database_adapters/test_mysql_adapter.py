...
class TestMySQLAdapter:
    ...
    def test_init_without_mysql_connector(self):
        """Test initialization when mysql-connector-python is not available"""
        with patch('tools.services.data_analytics_service.adapters.database_adapters.mysql_adapter.MYSQL_AVAILABLE', False):
            with pytest.raises(ImportError, match="mysql-connector-python is required"):
                MySQLAdapter()
    ...
    def test_execute_query_with_params(self, adapter):
        ...
        adapter._execute_query("SELECT * FROM test WHERE id = %s", (1,))
        
        adapter.cursor.execute.assert_called_once_with("SELECT * FROM test WHERE id = %s", (1,))
    ...
    def test_get_tables(self, adapter):
        ...
        with patch.object(adapter, '_execute_query', return_value=adapter.cursor.fetchall.return_value):
        ...
        assert tables[0].record_count == 100
    
    def test_get_tables_with_schema_filter(self, adapter):
        """Test getting tables with schema filter"""
        with patch.object(adapter, '_execute_query', return_value=[]):
            adapter.get_tables(['schema1', 'schema2'])
        
        # Verify that schema filter is applied in the query
        # This would require checking the actual SQL query generated
    ...
    def test_get_table_size(self, adapter):
        ...
        assert result['total_size_mb'] == 150.25
        assert result['table_rows'] == 10000
