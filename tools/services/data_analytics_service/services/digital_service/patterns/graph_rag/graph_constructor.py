...
class GraphConstructor(BaseService):
    ...
    async def construct_graph(self, 
        ...
        for entity_text, attributes in entity_attributes.items():
            ...
            if entity_id:
                # Handle attributes whether they are a list or dict
                if isinstance(attributes, list):
                    for attribute in attributes:
                        # For list of GenericAttribute objects
                        attr_name = getattr(attribute, 'name', 'unknown')
                        attr_value = getattr(attribute, 'normalized_value', None) or getattr(attribute, 'value', '')
                        attr_type_str = getattr(attribute, 'attribute_type', 'TEXT')
                        if not isinstance(attr_type_str, str):
                            attr_type_str = str(attr_type_str)
                        attr_text = f"{attr_name}: {attr_value} (type: {attr_type_str})"
    ...
    async def _construct_from_text_sequential(self, 
        ...
        from .entity_extractor import create_business_entity_extractor
        from .relation_extractor import create_business_relation_extractor  
        from .attribute_extractor import create_business_attribute_extractor
        
        # Initialize extractors
        entity_extractor = create_business_entity_extractor()
        relation_extractor = create_business_relation_extractor()
        attribute_extractor = create_business_attribute_extractor()
    ...
    def _create_entity_text(self, entity: Entity, attributes: Any) -> str:
        ...
        # Handle attributes whether they are a list or dict
        if isinstance(attributes, list):
            # If attributes is a list of GenericAttribute objects
            for attr in attributes:
                if hasattr(attr, 'confidence') and attr.confidence > 0.8:
                    # Use 'name' and 'normalized_value' or 'value' for GenericAttribute
                    attr_name = getattr(attr, 'name', 'unknown')
                    attr_value = getattr(attr, 'normalized_value', None) or getattr(attr, 'value', '')
                    text_parts.append(f"{attr_name}:{attr_value}")
        elif isinstance(attributes, dict):
            # If attributes is a dict
