...
class TestNeo4jStore:
    ...
    async def test_store_knowledge_graph(self, neo4j_store, sample_graph_data, mock_neo4j_client):
        ...
        # Note: attributes are handled separately, not in entity properties
        assert "canonical_form" in entity_call["properties"]
    ...
    async def test_large_graph_storage(self, neo4j_store, mock_neo4j_client):
        ...
        # Arrange
        large_graph_data = {
            "entities": [{"id": f"entity_{i}", "name": f"Entity {i}", "type": "TEST", "canonical_form": f"Entity {i}", "confidence": 0.9, "embedding": [0.1] * 1536} for i in range(100)],
            "relations": [{"id": f"rel_{i}", "source_id": f"entity_{i}", "target_id": f"entity_{i+1}", "type": "CONNECTS", "predicate": "connects_to", "confidence": 0.8, "embedding": [0.2] * 1536} for i in range(99)]
        }
...
class TestNeo4jStoreEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_malformed_graph_data(self, neo4j_store_with_failing_client):
        ...
        # Act
        storage_result = await neo4j_store_with_failing_client.store_knowledge_graph(malformed_data)
