...
if grep -q "quality" "${OUTPUT_DIR}/crag_store_knowledge_output.txt" 2>/dev/null; then
    echo "   ✅ Quality assessment found in store results"
else
    echo "   ⚠️  Quality assessment not found in store results"
fi

if grep -q "quality" "${OUTPUT_DIR}/crag_search_knowledge_output.txt" 2>/dev/null; then
...
if grep -q "crag" "${OUTPUT_DIR}/crag_store_knowledge_output.txt" 2>/dev/null; then
