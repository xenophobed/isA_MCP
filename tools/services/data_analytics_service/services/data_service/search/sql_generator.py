...
请生成准确的SQL查询，要求：
1. **【重要】只能使用上面"可用数据表结构"中明确列出的表名和字段名，不要使用任何未列出的字段**
2. **【重要】如果上面的表结构中没有用户查询所需的字段，请根据现有字段生成最接近的查询**
3. 使用正确的表连接关系
4. 确保字段名称准确无误，与上面结构中的名称完全一致
5. 正确处理时间范围和过滤条件
6. 使用合适的聚合函数和分组
7. 添加必要的性能优化（如LIMIT、索引提示）
8. 遵循{domain}业务规则和数据完整性约束
