...
class TestAttributeExtractor:
    ...
    async def test_extract_with_llm_success(self, mock_extract, extractor, sample_person_entity):
        ...
        assert result['age'].attribute_type == AttributeType.NUMBER
    ...
    @pytest.mark.asyncio
    @patch('tools.services.intelligence_service.language.text_extractor.text_extractor.extract_key_information')
    async def test_extract_attributes_success(self, mock_extract, extractor, sample_person_entity):
        ...
        assert result['age'].normalized_value == 30  # Should be normalized to int
    
    @pytest.mark.asyncio
    @patch('tools.services.intelligence_service.language.text_extractor.text_extractor.extract_key_information')
    async def test_extract_entity_attributes_batch(self, mock_extract, extractor, sample_person_entity, sample_organization_entity):
