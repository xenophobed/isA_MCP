...
from .metadata_extractor import TableInfo, ColumnInfo, RelationshipInfo
...
class SemanticAnalyzer(ABC):
    ...
    @abstractmethod
    async def analyze_schema(self, tables: List[TableInfo]) -> SemanticAnalysisResult:
...
class SQLSemanticAnalyzer(SemanticAnalyzer):
    ...
    async def analyze_schema(self, tables: List[TableInfo]) -> SemanticAnalysisResult:
        ...
        try:
            logger.info(f"Starting semantic analysis of {len(tables)} tables")
            
            # Discover business entities
            entities = await self._discover_business_entities(tables)
            
            # Detect data patterns
            patterns = self._detect_data_patterns(tables)
            
            # Infer business rules
            rules = self._infer_business_rules(tables, entities)
            
            # Generate schema insights
            insights = self._generate_schema_insights(tables, entities, patterns)
            
            # Assess data quality
            quality = self._assess_data_quality(tables, patterns)
            
            # Generate recommendations
            recommendations = self._generate_recommendations(entities, patterns, rules, quality)
            
            result = SemanticAnalysisResult(
                business_entities=entities,
                data_patterns=patterns,
                business_rules=rules,
                schema_insights=insights,
            ...
            raise SemanticAnalysisError(f"Failed to analyze schema: {str(e)}")
    
    async def _discover_business_entities(self, tables: List[TableInfo]) -> List[BusinessEntity]:
        """Extract semantic features from metadata using LLM analysis"""
        
        # Prepare metadata summary for LLM analysis
        tables = metadata.get("tables", [])
        columns = metadata.get("columns", [])
        
        analysis_prompt = f"""
        Analyze the following database metadata and extract semantic features:
        
        Tables ({len(tables)}):
        {json.dumps([{k: v for k, v in table.items() if k in ['table_name', 'table_type', 'record_count']} for table in tables[:10]], indent=2)}
        
        Columns (showing first 20):
        {json.dumps([{k: v for k, v in col.items() if k in ['table_name', 'column_name', 'data_type', 'is_nullable']} for col in columns[:20]], indent=2)}
        
        Extract these semantic features:
        1. Domain categories (e.g., e-commerce, finance, hr, logistics)
        2. Entity types (e.g., customer, product, transaction, event)
        3. Data patterns (e.g., time series, hierarchical, transactional)
        4. Naming conventions and standards
        5. Business process indicators
        
        Return as JSON with confidence scores (0.0-1.0) for each feature.
        """
        
        try:
            response = await self.llm.infer(analysis_prompt)
            semantic_features = json.loads(response)
            
            # Add embedding-based features
            embedding_features = await self._extract_embedding_features(metadata)
            semantic_features.update(embedding_features)
            
            return semantic_features
            
        except Exception as e:
            logger.warning(f"LLM semantic feature extraction failed: {e}")
            return {"error": str(e), "fallback_features": await self._extract_basic_features(metadata)}
