...
Future: Will support DuckDB for database storage
"""
...
class ApplicabilityExtractor:
    ...
    def __init__(self, doc_path: str):
        ...
        logger.info(f"   Tables found: {len(self.doc.tables)}")
    ...
    async def extract_applicability_table(self) -> Tuple[List[TestCase], Dict]:
        ...
        logger.info(f"✅ LLM identified {len(test_table_indices)} test tables: {test_table_indices}")
        
        if not test_table_indices:
            logger.warning("⚠️  No test tables identified by LLM")
            return [], {}
    ...
    def extract_pics_conditions(self) -> Tuple[List[Condition], Dict]:
        ...
        # Find all PICS tables
        for i, table in enumerate(self.doc.tables):
            if not table.rows:
                continue
            
            # Check if table contains pc_ mnemonics
            table_text = '\n'.join([
                ' '.join([str(cell) for cell in row if cell]) 
                for row in table.rows[:5]
            ])
            
            if 'pc_' not in table_text:
                continue
            
            # Use headers from Table object
            headers = table.headers
            
            # Find Mnemonic column
            mnemonic_col = -1
            ...
            for row_values in table.rows:
                ...
                # Get mnemonic
                if mnemonic_col >= 0 and mnemonic_col < len(row_values):
                    mnemonic = str(row_values[mnemonic_col]).strip() if row_values[mnemonic_col] else ''
                    if mnemonic and mnemonic.startswith('pc_'):
                        # Create condition
                        condition = Condition(
                            condition_id=mnemonic,
